#!/usr/bin/env python3
"""
Depression Level Prediction - Fast Execution Script
Auto-generated headless version of depression_level.ipynb for demonstration.
"""
...
print("="*60)
print("DEPRESSION LEVEL PREDICTION - FAST EXECUTION")
print("="*60)
...
if TARGET not in df.columns:
    raise KeyError(f'Missing target {TARGET}. Available: {list(df.columns)[:10]}')
for c in df.select_dtypes(include='object').columns:
    df[c] = df[c].astype(str).str.strip().replace({'nan':np.nan,'None':np.nan,'' : np.nan})
print(f'✓ Loaded data. Shape: {df.shape}')
print(f'✓ Target classes: {df[TARGET].unique()[:10]}')
...
# 5. Train Candidates
candidates={'logreg':LogisticRegression(max_iter=400),
...
for name,clf in candidates.items():
    ...
    results[name]=m
    models[name]=pipe
    print(f"  {name}: acc={m['accuracy']:.4f}, f1={m['f1_weighted']:.4f}, time={m['train_time_sec']}s")

# 6. Best Model & Persistence
...
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig(RESULTS_DIR/'depression_confusion_matrix.png', dpi=150)
plt.close()
print(f"✓ Saved confusion matrix to {RESULTS_DIR/'depression_confusion_matrix.png'}")

# Save artifacts
artifact={'model':best_model,
          'label_encoder':label_encoder,
          'classes':list(label_encoder.classes_),
          'metrics_table':results_df.to_dict(),
          'best_model_name':best_name,
          'timestamp':dt.datetime.utcnow().isoformat()}

joblib.dump(artifact, MODEL_DIR/'best_depression_model_fast.joblib')
results_df.to_csv(RESULTS_DIR/'depression_model_metrics_fast.csv')

# Sample predictions
sample=best_model.predict(X_test[:10])
sample_labels = label_encoder.inverse_transform(sample)
print(f"✓ Sample predictions: {sample_labels[:5]}")
print(f"✓ Saved artifacts: {MODEL_DIR/'best_depression_model_fast.joblib'}")
print(f"✓ Saved metrics: {RESULTS_DIR/'depression_model_metrics_fast.csv'}")

print("\n" + "="*60)
print("DEPRESSION PREDICTION COMPLETED SUCCESSFULLY")
print("="*60)
